...
namespace Serilog.Sinks.CustomFile
{
    sealed class RollingFileSink : ILogEventSink, IFlushableFileSink, IDisposable
    {
                               ...
                               TimeSpan? retainedFileTimeLimit)
        {
            ...
            _hooks = hooks;
            _currentFileSequence = GetDefaultStartingSequence();
        }

        private int? GetDefaultStartingSequence()
        {
            return _rollOnFileSizeLimit ? 1 : (int?)null;
        }
        ...
        void AlignCurrentFileTo(DateTime now, bool nextSequence = false)
        {
            ...
            {
                int? minSequence = GetDefaultStartingSequence();
                if (nextSequence)
                {
        ...
        void OpenFile(DateTime now, int? minSequence = null)
        {
            ...
            var sequence = latestForThisCheckpoint?.SequenceNumber ?? GetDefaultStartingSequence();
            if (minSequence != null)
            {
